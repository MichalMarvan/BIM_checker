...
Trigger: LinkedIn feedback from Petru Conduraru (buildingSMART trainer, RO).
```
...
### Notes
- Trigger: feedback from Petru Conduraru, buildingSMART trainer (RO)
...
git push
gh pr create --title "i18n: complete English localization (Petru feedback)" --body "..."
```

PR body should mention: Petru's LinkedIn feedback, scope (~166 fixes), acceptance criteria, manual test plan.
...
**Final state:** ~741 tests pass, regression test gates future regressions, EN UI shows zero CS text, Petru invited to retest.
